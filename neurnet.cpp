...
void drawgraph(SDL_Renderer* renderer, vector<double>* output, int pos, vector<double>* weight,char* argv2,int plusday, bool disablegraph = false) {
    SDL_Event event;
    if (!disablegraph)
    {
...
void computenextday2(SDL_Renderer* renderer, int argc, char* argv[]) {
    cout << "--- " << argv[4] << " - " << argv[1] << " ---" << endl;
    int plusday = stoi(argv[3]);
    char buffer[512];
    sprintf_s(buffer, "%s", argv[4]);
    ...
    drawgraph(renderer, &output, 0, &weight, argv[4], plusday,true);
    //compnextday(&input, &output, &weight);
    cout << datavect[datavect.size() - 1].close << " -> " << (output)[rows - 1] << " $" << endl;
    cout << (output)[rows - 1] - datavect[datavect.size() - 1].close << " $" << endl;
    cout << -(1 - (output)[rows - 1] / datavect[datavect.size() - 1].close) * 100 << " %" << endl;
